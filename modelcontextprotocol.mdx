...
List grouped security issues by CWE, with optional filters and pagination.
...
- `filters.urgency` (string, optional)
- `filters.status` (string, optional)
- `filters.confidence` (string, optional)
...
**Returns:**
Grouped security issues with counts per CWE, optionally including urgency and status breakdowns.
  ...
  "groups": [
    {
      ...
      "count": 5,
      "breakdown": {
        "urgency": { "high": 2, "medium": 3 },
        "status": { "open": 4, "in_progress": 1 }
      }
    }
  ]
}
