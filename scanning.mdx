...
User's have a choice between two options. The first being the Corgea Dropsite and the second being the Corgea CLI. 
<Info>
  **Prerequisite** You registered in Corgea

  **SAST Scanner** You have a SAST scanner like Snyk, Checkmarx, CodeQL or Semgrep.
</Info>
...
### Corgea CLI
The Corgea CLI (Command-Line Interface) provides advanced features for integrating Corgea with your CI/CD pipeline, performing custom scans, and uploading large projects (>1GB).
Click on the "Corgea CLI" accordion to learn more about installing and using the CLI.
