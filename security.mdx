...
<AccordionGroup>
  <Accordion icon="robot" title="Does Corgea utilize our proprietary data to enhance its AI models?">
    Your trust is paramount. Corgea is designed to respect your data’s confidentiality. We ensure that your data is not used to train our models, maintaining the integrity of your proprietary information.
  </Accordion>
  <Accordion icon="code" title="What happens to our code within Corgea’s system?">
    Protecting your intellectual property is our top priority. Corgea operates by storing only the code differences (diffs) necessary to facilitate fixes, handling them with industry-leading encryption standards during both storage and transmission.  
  </Accordion>
  ...
  <Accordion icon="biohazard" title="What measures does Corgea take to prevent prompt poisoning?">
    Corgea is fortified with rigorous output checks to prevent AI-generated anomalies. Coupled with mandatory human reviews during the PR process, we provide a balanced defense against prompt poisoning.
  </Accordion>
  ...
  <Accordion icon="house" title="Is Corgea deployed as a Single-tenant or Multi-tenant application?">
    To ensure data isolation and security, Corgea adopts a single-tenant deployment model. Each customer gets a dedicated environment with separate compute and storage resources. These environments are siloed, ensuring no cross-communication or data leakage between customers.
  </Accordion>
</AccordionGroup>
...
- Infrastructure Access Control: Access to Corgea’s infrastructure is managed through a corporate Virtual Private Network (VPN) - Tailscale. Tailscale provides a secure and encrypted connection, ensuring that access to internal resources is restricted to authorized personnel only.
...
- Tailscale Logging: All server access is logged within Tailscale. This includes information about who accessed the server, when the access occurred, and what actions were performed.
