...
1. Redirect users to Corgea's authorization page with a callback URL (for example: `/authorize/?callback=http%3A%2F%2Flocalhost%3A49215%2Fauth%2Fcallback`)
...
Callback URLs must use an approved host. Loopback callbacks (`localhost`, `127.0.0.1`, `::1`) are supported for local CLI flows, and Corgea IDE callbacks support approved custom schemes such as `cursor://Corgea.corgea/callback` and `vscode://Corgea.corgea/callback`.

If the callback is missing or invalid, the authorization endpoint returns `400 Bad Request`.
