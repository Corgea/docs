...
<Info>
  **Prerequisite** Ensure that you have configured the [Azure DevOps integration](azure_devops) with Corgea and have the necessary permissions set up.
</Info>

<Steps>
  <Step title="Pull Request Submission">
    Developer submits a pull request with code changes in Azure DevOps
  </Step>
  <Step title="Automated Analysis">
    System analyzes the code changes against active blocking rules configured in Corgea
  </Step>
  <Step title="Rule Validation">
    If any violations are detected, the pull request is automatically blocked
    <Frame>
      <img src="/images/azure_blocking_rules_pr_blocked.png" style={{ borderRadius: '0.5rem' }} />
    </Frame>
  </Step>
  <Step title="Developer Notification">
    Developer can click the link to see more detail about what issue has failed on Corgea Scan page.
    <Frame>
      <img src="/images/azure_see_details_of_issues.png" style={{ borderRadius: '0.5rem' }} />
    </Frame>
  </Step>
  <Step title="Resolution">
    Developer must address the violations by fixing them, or marking them as False Positive or Accepted Risk before the merge is allowed
  </Step>
</Steps>
