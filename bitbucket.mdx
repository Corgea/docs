...
<Steps>
  ...
  <Step title="Configure Bitbucket Pipelines (Corgea-only Scan)">
    If you would rather skip a dedicated SAST tool and let Corgea handle scanning end-to-end, use the following pipeline. It installs your Python dependencies, runs unit tests and linting, then executes `corgea scan` to analyse the repository. The remainder of the script waits for triage and pushes issue details back to Bitbucket Code Insights.
