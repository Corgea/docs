...
---
<Update label="2026.02.25" description="origin/main">
  **New Features:**
  - Added login audit log APIs for successful and failed authentication events, including company-wide and per-user views.
  - Introduced user login and login failed webhook events for real-time auth monitoring. — [Learn more](/webhooks)
  - Added SCA reachability analysis for direct dependencies, including per-file usage views and triage status badges.

  **UI & Experience:**
  - Added reachability and dependency depth filters in SCA findings and CSV exports.
  - Simplified vulnerability report filters by removing confidence filtering and related labels.

  **Improvements:**
  - Project deletion now requires explicit permissions and access control checks. — [Learn more](/content-access-management)
  - Improved Azure DevOps repository URL normalization for more consistent project identification.

  **Reliability & Stability:**
  - Webhook delivery now runs asynchronously for more reliable and scalable event processing. — [Learn more](/webhooks)
