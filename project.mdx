...
<Note>Changing scan settings requires the **Can edit project** permission. Users without project edit access can review existing ignore rules and CWE filters where shown, but cannot add, update, or delete them.</Note>
...
<Steps>
  ...
  <Step title="Pick the CWEs you want to ignore">
    Add a CWE ID and optionally include a **Glob Pattern** to scope the ignore rule to specific files or folders (for example, `**/tests/**`). Leave the glob pattern empty to apply the rule across all files in the project.
    <Card>
